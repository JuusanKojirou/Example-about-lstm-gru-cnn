...
def contextwin(l, win):
    ...
    if (isinstance(l[0],(numpy.int32))):
        ...
        for i in range(len(l)):
            out += [0]
            w = list(l[i])
            lpadded = win // 2 * [-1] + w + win // 2 * [-1]
            out_temp = [lpadded[j:(j + win)] for j in range(len(l[i]))]
            out[i] = out_temp
        assert len(out) == len(l)
    return out
# end-snippet-1
...
class RNNSLU(object):
    ...
    def __init__(self, nh, nc, ne, de, cs):
        ...
        # start-snippet-3
        idxs = T.itensor3()
        x_x = self.emb[idxs].flatten(3)
        x = x_x.dimshuffle(1,0,2)
        y_sentence = T.imatrix('y_sentence')  # labels
        ...
        [h, s], _ = theano.scan(fn=recurrence,
                                sequences=x,
                                outputs_info=[T.dot(T.alloc(numpy.asarray(1., dtype=theano.config.floatX),x.shape[1],1), self.h0.reshape((1,nh))), None],
                                n_steps=x.shape[0])

        p_y_given_x_sentence = s.dimshuffle(1,0,2)
        ...
        [i,nll_s],_ = theano.scan(fn=lambda t1,i1:[i1+1,-T.mean(T.log(p_y_given_x_sentence[i1])[T.arange(y_sentence.shape[1]),t1])],
                                  sequences=y_sentence,
                                  outputs_info=[0,None],
                                  n_steps=y_sentence.shape[0])                                 
        sentence_nll = T.mean(nll_s)
    ...
    def train(self, x, y, window_size, learning_rate):
        ...
        labels = list(map(lambda x: numpy.asarray(x).astype('int32'), y))
...
def main(param=None):
    ...
    def padding(padding_num,dataset,padding_length):
        for i in range(len(dataset)):
            temp = padding_num*numpy.ones(padding_length)
            pad = min(padding_length,len(dataset[i]))
            temp[:pad] = dataset[i][:pad]
            dataset[i] = temp
        return dataset
    
    train_lex = padding(7,train_lex_temp,param['padding_len'])
    train_y = padding(126,train_y_temp,param['padding_len'])

    n_train_batches = len(train_lex) // param['batchsize']
    ...
    for e in range(param['nepochs']):
        ...
        for minibatch_index in range(n_train_batches):
            ...
            rnn.train(train_lex[minibatch_index  * param['batchsize']: (minibatch_index  + 1) * param['batchsize']],
                      train_y[minibatch_index  * param['batchsize']: (minibatch_index  + 1) * param['batchsize']],
                      param['win'], param['clr'])
            print('[learning] epoch %i >> %2.2f%%' % (
                    e, (minibatch_index + 1) * 100. / n_train_batches), end=' ')
        ...
        predictions_test = [map(lambda x: idx2label[x],
                            ...
                            contextwin(x, param['win'])).astype('int32'))[0])
                            for x in test_lex]
