...
from LSTM_utils import contextwin
...
class RNNSLU(object):
    ...
    def __init__(self, nh, nc, ne, de, cs, normal=True):
        ...
        def recurrence(x_t, h_tm1):
            at  = T.dot(x_t, self.wx) + T.dot(h_tm1, self.wh)
            ut  = T.mean(at)
            sigma = T.sqrt(T.mean(T.sqr(at-ut)))

            h_t = T.nnet.sigmoid((g/sigma)*(at-ut)+self.bh)
...
def train_nn2(train_model, validate_model, test_model,
    ...
    improvement_threshold = 0.9995  # a relative improvement of this much is
                                   # considered significant
    validation_frequency = min(100,n_train_batches, patience // 2)
                                  # go through this many
                                  # minibatche before checking the network
                                  # on the validation set; in this case we
                                  # check every epoch
